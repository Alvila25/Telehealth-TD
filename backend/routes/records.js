...
router.post('/upload', authMiddleware, upload.single('file'), async (req, res) => {
  ...
  await User.update(
    { medicalHistory, telehealtData: telehealtData ? JSON.parse(telehealtData) : null },
...
router.get('/patient/:id', authMiddleware, async (req, res) => {
  ...
  const patient = await User.findByPk(req.params.id, { attributes: ['medicalHistory', 'telehealtData'] });
  res.json(patient);
});
