console.log('Starting app.js, yo!'); // Confirm file is even touched
...
// Import core modules with explicit checks
try {
  const express = require('express');
  const http = require('http');
  const { Server } = require('socket.io');
  const { Sequelize } = require('sequelize');
  console.log('Core modules loaded, we’re good!');
  ...
  console.error('Core module load failed:', err);
  process.exit(1);
}
